/** Name-matched product images — each URL chosen to match the cake name. */
const IMG = (id) =>
  `https://images.unsplash.com/photo-${id}?w=800&q=80&auto=format&fit=crop`;

const CAKE_IMAGE_IDS = {
...
function imageUrlForCakeName(name, fallbackUrl = '') {
  const id = CAKE_IMAGE_IDS[name];
  if (id) return IMG(id);
  const trimmed = (fallbackUrl || '').trim();
  return trimmed || null;
}

const CAKE_IMAGES = Object.fromEntries(
  Object.entries(CAKE_IMAGE_IDS).map(([name, id]) => [name, IMG(id)])
);
